...
import { Edit, Book, Hash, GraduationCap } from "lucide-react";
...
export default function EditSubjectModal({ 
    ...
    const handleSubmit = async () => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.put(`/api/admin/subjects/${subject.id}`, {
            ...
            if (response.data.status === 200) {
                ...
                addToast({
                    color: "danger",
                    title: "ผิดพลาด",
                    description: response.data.message || "ไม่สามารถแก้ไขวิชาได้"
                });
            }
        } catch (error: any) {
            ...
            addToast({
                color: "danger",
                title: "ผิดพลาด",
                description: error.response?.data?.message || "เกิดข้อผิดพลาดในการแก้ไขวิชา"
            });
        } finally {
            setIsLoading(false);
        }
        ...
        >
            <ModalContent>
                <ModalHeader className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-gray-800">แก้ไขวิชา</h2>
                        <p className="text-sm text-gray-600">แก้ไขข้อมูลวิชา: {subject.name}</p>
                    </div>
                </ModalHeader>
