...
import { Edit, BookOpen, FileText, GraduationCap, Calendar, Camera } from "lucide-react";
...
export default function EditBookModal({
    ...
    useEffect(() => {
        if (book && isOpen) {
            setFormData({
                ...
                subject_id: book.subject.id.toString(),
                academic_year_id: book.AcademicYear.id.toString()
            });
            setSubjectInputValue(`${book.subject.code} - ${book.subject.name}`);
        }
    }, [book, isOpen]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.put(`/api/admin/books/${book.id}`, {
            ...
            if (response.data.status === 200) {
                addToast({
                    color: "success",
                    title: "สำเร็จ",
                    description: "แก้ไขข้อมูลหนังสือสำเร็จ"
                });
                handleClose();
                onSuccess();
            } else {
                addToast({
                    color: "danger",
                    title: "ผิดพลาด",
                    description: response.data.message || "ไม่สามารถแก้ไขข้อมูลหนังสือได้"
                });
            }
        } catch (error: any) {
            console.error("Error updating book:", error);
            const errorMessage = error.response?.data?.message || "เกิดข้อผิดพลาดในการแก้ไขข้อมูลหนังสือ";
            addToast({
                color: "danger",
                title: "ผิดพลาด",
            ...
            >
                <ModalContent>
                    <ModalHeader className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-gray-800">แก้ไขข้อมูลหนังสือ</h2>
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                <div className="space-y-2">
                                    <div className="flex gap-2">
                                        ...
                                        <div className="flex flex-col justify-end">
                                            <Button
                                                ...
                                                className="border-blue-300 text-blue-600 hover:bg-blue-50 h-14"
                                                title="สแกนบาร์โค้ด"
                                            >
                                    ...
                                    <div className="flex items-center gap-2 text-sm">
                                        ...
                                        <span className="text-blue-600">คลิกปุ่มกล้องเพื่อสแกน</span>
                                        {formData.barcode && (
                                            <>
                            ...
                            {(getSelectedSubject() || getSelectedAcademicYear()) && (
                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                    <p className="text-sm text-blue-600 mb-2">ตัวอย่างข้อมูลหนังสือ:</p>
                            ...
                            <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
                                <h4 className="font-medium text-amber-800 mb-2">ข้อมูลเพิ่มเติม</h4>
                                <div className="text-sm text-amber-700 space-y-1">
                                    <p>• รหัสหนังสือต้องไม่ซ้ำกับหนังสือที่มีอยู่แล้ว</p>
                                    <p>• การแก้ไขจะมีผลกับการลงทะเบียนที่มีอยู่</p>
                                    <p>• หากมีนักเรียนลงทะเบียนแล้ว ควรระมัดระวังในการแก้ไข</p>
                                </div>
                            </div>
                        </div>
                    </ModalBody>
